-- Migration: Fix trigger race condition and add microsecond precision
-- Date: 2026-05-08
-- Issue: queue_joined_at not resetting when courier goes off → on
-- Root Cause: Checking NEW.is_online after modifying it (race condition)
...
-- Add comment
COMMENT ON FUNCTION public.handle_courier_queue_sync IS 'Manages courier queue sync with fixed race condition and microsecond precision';;
