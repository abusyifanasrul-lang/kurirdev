...
BEGIN
  ...
  v_today     := v_now_local::DATE;  -- ✅ tanggal lokal, bukan UTC

  -- Cek sudah check-in hari ini belum
  SELECT EXISTS(
    SELECT 1 FROM shift_attendance
    WHERE courier_id = p_courier_id
      AND date = v_today  -- ✅ fix: pakai v_today
  ...
  INSERT INTO shift_attendance (
    ...
    p_courier_id, v_shift.id, v_today,  -- ✅ fix: pakai v_today
    NOW(), v_late_minutes,
