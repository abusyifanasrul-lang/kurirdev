-- Migration: Fix "Already Checked-in" Blocking Courier from Going Online
-- Date: 2026-06-02
-- Description: Add p_skip_duplicate_check parameter to allow couriers to go online
--              even after they've already checked in for attendance tracking
-- 
-- Bug: When courier checks in, then goes OFF, then tries to go ON again,
--      the system rejects with "already_checked_in" error, preventing them
--      from receiving orders.
-- 
-- Fix: Add optional parameter to skip duplicate check when just toggling online status

-- ============================================================================
-- Function: record_courier_checkin (UPDATED)
-- Purpose: Allow resume online after check-in without duplicate error
-- New Parameter: p_skip_duplicate_check (default FALSE for backward compatibility)
-- ============================================================================
...
BEGIN
  ...
  -- NEW LOGIC: Handle already checked-in case
...
COMMENT ON FUNCTION public.record_courier_checkin(UUID, BOOLEAN) IS 
  'Records courier check-in with shift window validation. Set p_skip_duplicate_check=true to allow resume online after already checked-in.';
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 20260602080000 completed successfully';
  RAISE NOTICE 'Updated function: record_courier_checkin()';
  RAISE NOTICE 'Added parameter: p_skip_duplicate_check (default FALSE)';
  RAISE NOTICE 'Fix: Couriers can now go online after check-in without duplicate error';
END $$;
