...
**Note:** `src/types/supabase.ts` reflects generated types; actual PostgreSQL `CHECK`, `DEFAULT`, and triggers may differ slightly — cross-check `supabase/migrations/` for authoritative DDL.
...
4. [Database schema from `src/types/supabase.ts`](#4-database-schema--expanded)  
...
10. [Appendix A — Full multi-tenant SQL migration](#appendix-a--full-multi-tenant-sql-migration)
...
## Appendix A — Full multi-tenant SQL migration
...
3. **This script is destructive to RLS policy names** — test on a branch DB first.

```sql
-- =============================================================================
-- Multi-tenancy migration — KurirMe (template)
-- Apply in staging first. Adjust policy names if your DB diverges.
-- =============================================================================

BEGIN;

-- 1) Core org & billing tables
...
CREATE TABLE IF NOT EXISTS public.plans (
  ...
  max_couriers int NOT NULL DEFAULT 10,
  max_orders_per_month int,
  max_basecamps int,
  max_scheduled_notifications_per_day int,
...
-- 2) Seed plans (example)
INSERT INTO public.plans (slug, name, max_members, max_couriers, max_orders_per_month, features)
VALUES
  ('free', 'Free', 5, 10, 500, '{"finance_module": true, "stay_qr": false}'::jsonb),
  ('pro', 'Pro', 50, 100, NULL, '{"finance_module": true, "stay_qr": true, "bulk_csv_export": true}'::jsonb),
  ('enterprise', 'Enterprise', NULL, NULL, NULL, '{"finance_module": true, "stay_qr": true, "sso": true}'::jsonb)
ON CONFLICT (slug) DO NOTHING;
...
BEGIN
  ...
  INSERT INTO public.subscriptions (organization_id, plan_id, status)
  SELECT v_org, v_plan, 'active'
  WHERE NOT EXISTS (SELECT 1 FROM public.subscriptions s WHERE s.organization_id = v_org);

  INSERT INTO public.organization_members (organization_id, user_id, role)
  SELECT v_org, p.id,
    CASE WHEN p.role IN ('owner','admin') THEN 'owner' ELSE 'member' END
  FROM public.profiles p
  ON CONFLICT (organization_id, user_id) DO NOTHING;
END $$;

-- 4) Add organization_id to tenant tables (run once per table)
...
-- Repeat SET NOT NULL for other tables once verified

-- 7) settings: drop global-only assumption — unique per org
CREATE UNIQUE INDEX IF NOT EXISTS uq_settings_org ON public.settings(organization_id);

-- 8) FCM tokens (tenant-scoped; migrate off profiles.fcm_token over time)
...
-- Optional backfill from legacy profiles.fcm_token (run after profiles.organization_id populated)
-- INSERT INTO public.fcm_tokens (organization_id, user_id, token, platform, updated_at)
-- SELECT p.organization_id, p.id, trim(p.fcm_token),
--   CASE WHEN lower(coalesce(p.platform,'web')) = 'android' THEN 'android' ELSE 'web' END,
--   coalesce(p.fcm_token_updated_at, now())
-- FROM public.profiles p
-- WHERE p.fcm_token IS NOT NULL AND length(trim(p.fcm_token)) > 0
-- ON CONFLICT (organization_id, user_id, platform)
...
CREATE POLICY profiles_insert_self ON public.profiles FOR INSERT TO authenticated
  WITH CHECK (id = auth.uid() AND organization_id = public.request_org_id());
