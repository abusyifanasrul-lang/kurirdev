...
export function FinancePenagihan() {
  ...
  const fetchAllCourierFines = useCallback(async () => {
    // Only admin/manager/owner/finance roles can access fine data — skip if already denied
    if (finesAccessDenied || !user || !['admin', 'owner', 'admin_kurir', 'finance'].includes(user.role)) {
